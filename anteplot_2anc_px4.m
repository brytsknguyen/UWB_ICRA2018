...
flightdata = csvread('bagcsv/niv20170813_sq_90_15.csv', 1, 0);
setpoints = csvread('square_90_15.csv'); setpoints = setpoints';
...
vcPAdj = vcP;
vcPAdj(3, :) = vcPAdj(3, :);
rmse = sqrt(sum(sum((vcPAdj -px4P).^2, 2), 1)/K);

madx = mad(vcP(1, flightI) - px4P(1, flightI));
mady = mad(vcP(2, flightI) - px4P(2, flightI));
madz = mad(vcP(3, flightI) - px4P(3, flightI));
...
madyaw = mad(vcEul(3, :)*180/pi - px4Eul(3, :)*180/pi);
...
err = round([rmsex, madx, rmsey, mady, rmseyaw, madyaw], 3)
f = [];
Y = [];

figure('position', [1929 9 944 988]);
for m = 1:2
    for n = 1:2
        subplot(edgeTotal/2, 1, (m-1)*2 + n);
        I = (find(rqstrId == m & mobAntId == n & rspdrId == 1));
        Y = [Y; (uwbD(I) - vcD(I))'];
        f = [f, mean(Y)];
        plot(t(I), vcD(I), 'r', t(I), uwbD(I) - f(end));
    end
end